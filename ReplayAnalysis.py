...
class ReplayGUI:
    def __init__(s) -> None:
        s.window = tk.Tk("Replay Analysis")
        s.window.title("Rocket League Replay Analysis")
        #s.window.geometry("1000x800")
        s.CreateTabs()
        s.PopulateReplayTab()
        s.PopulateMatchTab()
        ...
        s.window.mainloop()
    def CreateTabs(s):
        s.tabParent = ttk.Notebook(s.window)

        s.replayTab = ttk.Frame(s.tabParent)
        s.tabParent.add(s.replayTab, text = "Replay List")

        s.matchTab = ttk.Frame(s.tabParent)
        s.tabParent.add(s.matchTab, text = "Match Details")

        s.playerTab = ttk.Frame(s.tabParent)
        s.tabParent.add(s.playerTab, text = "Player Details")

        s.teamTab = ttk.Frame(s.tabParent)
        s.tabParent.add(s.teamTab, text = "Team Details")

        s.gameTab = ttk.Frame(s.tabParent)
        s.tabParent.add(s.gameTab, text = "Game Details")
    def PopulateReplayTab(s):
        s.loadEngineB = tk.Button(s.replayTab, text = "Load Replays", command = s.BCreateReplayEngine)
        s.loadEngineB.pack()
    def PopulateMatchTab(s):
        s.loadMatchB = tk.Button(s.matchTab, text = "Load Match", command = s.BAnalyseMatch)
        s.loadMatchEntry = tk.Entry(s.matchTab)
        s.analysisTypeSelect = tk.Listbox(s.matchTab, selectmode="single")
        s.topRelevanceE = tk.Entry(s.matchTab)
        s.tagsEntry = tk.Entry(s.matchTab)

        s.topRelevanceE.insert(tk.END, "5")
        s.tagsEntry.insert(tk.END, "{Match Tags}")
        
        s.loadMatchEntry.pack(side = tk.LEFT)
        s.loadMatchB.pack(padx = 5, side = tk.LEFT)
        s.topRelevanceE.pack(side = tk.BOTTOM)
        s.analysisTypeSelect.pack(side = tk.BOTTOM, expand = tk.YES)#, fill = "both")
        s.tagsEntry.pack(side = tk.BOTTOM)

        analyseTypes = ["top", "average"]
        for type in analyseTypes:
            s.analysisTypeSelect.insert(tk.END, type)
        
    def BCreateReplayEngine(s):
        if not s.eL:
            s.CreateReplayEngine()
            s.eL = True
        s.loadEngineB.destroy()
        matchID = tk.Entry(s.replayTab, width = 10, fg = "blue")
        matchID.grid(row = 0, column = 0)
        matchID.insert(0, "MatchID")

        replayName = tk.Entry(s.replayTab, width = 15, fg = "blue")
        replayName.grid(row = 0, column = 1)
        replayName.insert(0, "Game ID")
            
        matchType = tk.Entry(s.replayTab, width = 10, fg = "blue")
        matchType.grid(row = 0, column = 2)
        matchType.insert(0, "Match Type")

        tagOne = tk.Entry(s.replayTab, width = 10, fg = "blue")
        tagOne.grid(row = 0, column = 3)
        tagOne.insert(0, "Tag One")

        tagTwo = tk.Entry(s.replayTab, width = 10, fg = "blue")
        tagTwo.grid(row = 0, column = 4)
        tagTwo.insert(0, "Tag Two")

        tagThree = tk.Entry(s.replayTab, width = 10, fg = "blue")
        tagThree.grid(row = 0, column = 5)
        tagThree.insert(0, "Tag Three")

        tagFour = tk.Entry(s.replayTab, width = 10, fg = "blue")
        tagFour.grid(row = 0, column = 6)
        tagFour.insert(0, "Tag Four")

        tagFive = tk.Entry(s.replayTab, width = 10, fg = "blue")
        tagFive.grid(row = 0, column = 7)
        tagFive.insert(0, "Tag Five")
        for i, match in enumerate(s.replayEngine.matches[:-30:-1]):
            match : Match
            j = i + 1
            matchID = tk.Entry(s.replayTab, width = 10, fg = "blue")
            matchID.grid(row = j, column = 0)
            matchID.insert(j, str(match.mL[0]))

            replayName = tk.Entry(s.replayTab, width = 15, fg = "blue")
            replayName.grid(row = j, column = 1)
            replayName.insert(j, str(match.mL[1]))
            
            matchType = tk.Entry(s.replayTab, width = 10, fg = "blue")
            matchType.grid(row = j, column = 2)
            matchType.insert(j, str(match.mL[3]))

            tagOne = tk.Entry(s.replayTab, width = 10, fg = "blue")
            tagOne.grid(row = j, column = 3)
            tagOne.insert(j, str(match.mL[-6]))

            tagTwo = tk.Entry(s.replayTab, width = 10, fg = "blue")
            tagTwo.grid(row = j, column = 4)
            tagTwo.insert(j, str(match.mL[-5]))

            tagThree = tk.Entry(s.replayTab, width = 10, fg = "blue")
            tagThree.grid(row = j, column = 5)
            tagThree.insert(j, str(match.mL[-4]))

            tagFour = tk.Entry(s.replayTab, width = 10, fg = "blue")
            tagFour.grid(row = j, column = 6)
            tagFour.insert(j, str(match.mL[-3]))

            tagFive = tk.Entry(s.replayTab, width = 10, fg = "blue")
            tagFive.grid(row = j, column = 7)
            tagFive.insert(j, str(match.mL[-2]))
    
    def CreateReplayEngine(s):
        s.replayEngine = ReplayAnalysis()

    def BAnalyseMatch(s):
        matchID = int(s.loadMatchEntry.get())
        analyseMatch = [x for x in s.replayEngine.matches if x.mL[0] == matchID][0]
        ...
        if tags != "{Match Tags}":
            ...
            tags = False
        analysisType = s.analysisTypeSelect.get()
        extraTopRelevance = int(s.topRelevanceE.get())
        matchAnalysed = s.replayEngine.AnalyseNode(analyseMatch, "matches", analysisType, extraTopRelevance, tags)

        nodes = sorted(matchAnalysed.analysisNodes, reverse = True, key = lambda x : abs(x.cR))
        s.DisplayResults(nodes)
    def DisplayMatchResults(s, nodes, sortBy = "cR"):
        s.loadMatchB.destroy()
        s.loadMatchEntry.destroy()
        s.analysisTypeSelect.destroy()
        s.topRelevanceE.destroy()
        s.tagsEntry.destroy()
        if sortBy != "cR":
            nodes.sort(reverse = True, key = lambda x : x.__dict__[sortBy])
