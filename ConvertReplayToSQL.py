...
def GetBallchasingDict(ballchasingAPI: ballchasing.Api, latestReplay):
	...
	replayFile.close()
	while True:
		output = ballchasingAPI.get_replay(replayID)
...
def HandleReplay(replay, ballchasingAPI, matchIndex, playerIndex, cur, dbConn, tags = []):
	...
	#		print(f"Duplicate Replay")
	#		matchData[1].value = f"{matchData[1].value}::{p}"
			#return matchIndex - 1, originalPlayerIndex, False
	try:
		cur.execute(f"""INSERT INTO matchTable ({','.join([x.databaseField for x in matchData])}) VALUES ({','.join([f'"{x.value}"' if type(x.value) == str else f"{str(x.value)}" for x in matchData])})""")
	except sqlite3.IntegrityError:
		print(f"Duplicate Replay")
		return matchIndex - 1, originalPlayerIndex, "duplicate"
	try:
...
def FindReplay(replayGroup):
	for file in os.listdir(replayGroup):
		if ".replay" in file:
			return f"{replayGroup}\{file}"
		else:
			output = FindReplay(f"{replayGroup}\{file}")
			if output:
				return f"{output}"
	return None
def main(replayFolder = r"C:\\Users\\tom\\AppData\\Roaming\\bakkesmod\\bakkesmod\\data\\replays\\", tags = []):
	...
	while True:
		if old:
			...
			replay = FindReplay(EURCgroupFolder)
			replayName = replay.split("\\")[-1]
			print(f"Processing Replay: {replay}")
			if replay:
				matchIndex, playerIndex, eType = HandleReplay(replay, ballchasingAPI, matchIndex, playerIndex, cur, dbConn, tags = ["EURC2022", "playoffs"])
				if eType == "player":
					print("player error")
					input()
					move(replay, rf"E:\Rocket League Replays\EURC Replay Group\Completed Playoffs\Fail\{replayName}--{matchIndex}.replay")
					...
					move(replay, rf"E:\Rocket League Replays\EURC Replay Group\Completed Playoffs\Success\{replayName}--{matchIndex}.replay")

			else:
				raise ValueError("No Replays Left")
			sleep(5)
